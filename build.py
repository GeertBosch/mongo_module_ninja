        ...
        if self.globalEnv.ToolchainIs('gcc', 'clang'):
            if 'CXX' in self.tool_commands:
                ninja.rule('CXX',
                    deps = 'gcc',
                    depfile = '$out.d',
                    command = '$CCACHE %s -MMD -MF $out.d'%(self.tool_commands['CXX']),
                    description = 'CXX $out')
            if 'SHCXX' in self.tool_commands:
                ninja.rule('SHCXX',
                    deps = 'gcc',
                    depfile = '$out.d',
                    command = '$CCACHE %s -MMD -MF $out.d'%(self.tool_commands['SHCXX']),
                    description = 'SHCXX $out')
            if 'CC' in self.tool_commands:
                ninja.rule('CC',
                    deps = 'gcc',
                    depfile = '$out.d',
                    command = '$CCACHE %s -MMD -MF $out.d'%(self.tool_commands['CC']),
                    description = 'CC $out')
            if 'SHCC' in self.tool_commands:
                ninja.rule('SHCC',
                    deps = 'gcc',
                    depfile = '$out.d',
                    command = '$CCACHE %s -MMD -MF $out.d'%(self.tool_commands['SHCC']),
